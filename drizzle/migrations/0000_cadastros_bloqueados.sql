ALTER TABLE public.perfis ADD COLUMN IF NOT EXISTS bloqueado_em timestamptz;
...
begin
  ...
  set aprovado = _aprovado,
      ...
      bloqueado_em = case when _aprovado then null else now() end,
...
CREATE FUNCTION public.adm_listar_acessos()
 RETURNS TABLE(id uuid, email text, nome text, telefone text, professor boolean, adm boolean, aprovado boolean, criado_em timestamp with time zone, documentos_fisicos boolean, bloqueado_em timestamp with time zone)
 LANGUAGE plpgsql
