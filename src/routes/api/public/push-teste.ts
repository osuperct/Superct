...
export const Route = createFileRoute("/api/public/push-teste")({
  server: {
    handlers: {
      ...
      POST: async ({ request }) => {
        ...
        if (corpo.token) {
          ...
          lista = Array.from(new Set((tokens ?? []).map((t) => t.token))).filter(Boolean);
        }
          ...
          lista.map(async (tokenPush) => {
            try {
              const res = await fetch(`${GATEWAY_URL}/v1/projects/_/messages:send`, {
                ...
                body: JSON.stringify({
                  message: {
                    ...
                    notification: {
                      title: "Super CT",
                      body: "Notificações ativadas! Você vai receber os recados aqui.",
                    },
                  },
                }),
              });
              if (res.ok) {
                ...
                falhas++;
                ultimoErro = `[${res.status}] ${await res.text().catch(() => "")}`;
                console.error("Falha no push de teste:", ultimoErro);
              }
            } catch (e) {
              falhas++;
        ...
        return json({ enviados, falhas, total: lista.length, ...(ultimoErro ? { detalhe: ultimoErro } : {}) });
      },
    },
  },
});
